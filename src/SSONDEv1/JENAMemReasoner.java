 ...
 * @since 2011-11-15
 * 
 * This class implements the similarity.OntologyModel interface for defining in-memory JENA model, model might support forward chaining reasoning, 
 *  
 *  
 */

//TODO we should keep methods which access to the extensional part of model separate from methods pertaining to the schema/vocabulary level, for example we might have ExtensionalOntologyModel and ExternalOntologyModel super interface on the current OntologyModel 
public class JENAMemReasoner implements OntologyModel {
	Model	model;	
	InfModel infModel;
	 ...
	 */
	public JENAMemReasoner( ArrayList <String> RDFDocumentURIs, String ruleUri, Boolean reasonerActive) {
		...
		for ( String uri :RDFDocumentURIs){	
			...
			if (Debug.printDownLoadedURI)
				System.out.println("The " + uri + " has been retrieved by JENAMem.Constructor!");
		}
	...
	public void loadURIs(ArrayList<String> URIs) {
		...
		for (String uri : URIs) {
			...
			if (Debug.printDownLoadedURI)
				System.out.println("The " + uri + " has been retrieved by JENAMem.loadUri!");
		}
	}		
