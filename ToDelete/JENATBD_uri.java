...
public class JENATBD_withURI implements OntologyModel {
	Model	model;	
	InfModel infModel;
	 ...
	 *
	 */
	public JENATBD_uri(String directory,  ArrayList <String> RDFDocumentURIs, String ruleUri, Boolean reasonerActive) {
		
		//TDB.getContext().set(TDB.symUnionDefaultGraph, true);
		Dataset dataset = TDBFactory.createDataset(directory) ;
		// first we try to treat it as triplestore with one model only, the default one, That is ok with generic triple TDB triple store 
		model= dataset.getDefaultModel();
		// if the model has no concrete statements, than it is probably a quad store and we'd better read the Union Graph
		if (model.size()<1) {
			Model model1=TDBFactory.createModel();;
			for ( String uri :RDFDocumentURIs){	
				
				model1.read(uri);
				//dataset.addNamedModel(uri,model1);
				if (Debug.printDownLoadedURI)
					System.out.println("The " + uri + " has been retrieved by JENATBD.Constructor!");
			}
			
			model= dataset.getNamedModel("urn:x-arq:UnionGraph").add(model1);
		}
		else{
			for ( String uri :RDFDocumentURIs){	
				model.read(uri);
...
//		}
//		
		//model= dataset.getNamedModel("urn:x-arq:UnionGraph");
	...
	throws Exception {
		...
		if (Debug.printDebugSparqlOntologyModel) {
			System.out.println("getAttributeValue for instance "+ instance +" and attribute " +attribute+" returns");
			for (String e : r) {
	...
	public ArrayList<String> getRelationValues(String instance, String relation) {
		...
		if (Debug.printDebugSparqlOntologyModel) {
			System.out.println("getrelationValue for instance "+ instance +" and relation " +relation+" returns");
			for (String e : r) {
				System.out.println(e + " ");
			}
			System.out.println();
